"""SONAR-Full: dual XLSR encoders + RFE (constrained SRM) + cross-attention
fusion + AASIST + Jensen--Shannon alignment loss (paper Sec. 4)."""
...
class SSLModel(nn.Module):
    def __init__(self, device, noise=False, xlsr_ckpt: str | None = None):
        super().__init__()
        import os
        from sonar.model import _resolve_xlsr_path
        cp_path = _resolve_xlsr_path(xlsr_ckpt)
        if not os.path.exists(cp_path):
            raise FileNotFoundError(
                f"XLSR-300M checkpoint not found at {cp_path}. "
                "Set SONAR_XLSR_CKPT or run scripts/download_checkpoints.py.")
...
class GuidedModel(nn.Module):
    def __init__(self, args,device):
        ...
        self.cross_attention_fuse = CrossAttentionFusion()
        
        self.LL = nn.Linear(self.noise_ssl_model.out_dim, 128)
        ...
        #self.noise_weight = nn.Parameter(torch.tensor(0.5),requires_grad=True)  # Start with 0.5
    ...
    def forward(self, x):
        ...
        x = self.cross_attention_fuse(x_content_ssl_feat,x_noise_ssl_feat)
        ...
        return output,x_low_copy,x_high_copy
